# Comprehensive installer for cognis-digital/enrichr (Windows PowerShell).
# Tries: pipx -> uv -> pip (git+https) -> from source.
# enrichr is source-available and not on PyPI; all paths install from GitHub.
$ErrorActionPreference = "Stop"
$Repo = "enrichr"
$Url  = "git+https://github.com/cognis-digital/enrichr.git"
$Git  = "https://github.com/cognis-digital/enrichr.git"
...
if (Have pipx) {
  ...
  pipx install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: enrichr"; exit 0 }
}
if (Have uv) {
  Say "Installing with uv..."
  uv tool install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: enrichr"; exit 0 }
}
if (Have pip) {
  Say "Installing with pip (user site)..."
  pip install --user $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: enrichr"; exit 0 }
}
